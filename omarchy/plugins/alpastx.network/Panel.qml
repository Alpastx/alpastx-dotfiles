...
Panel {
  ...
  // Bar label: fa-wifi / ethernet / disconnected icons; bandwidth rates use
  // theme bar foreground (BarWidget.qml), not legacy Waybar palette.
  readonly property string barIcon: {
    if (kind === "wifi") return ""
    if (kind === "ethernet") return "󰈀"
    return "󰖪"
  }
  readonly property bool showBarBandwidth: kind === "wifi" || kind === "ethernet"
  readonly property string barDownloadText: formatRateCompact(downloadRate)
  readonly property string barUploadText: formatRateCompact(uploadRate)
  readonly property string barLinkedText: {
    if (kind === "disconnected" || info.ip) return ""
    if (info.iface) return "󰈀 " + info.iface + " (No IP)"
    return ""
  }
  readonly property string barTooltipText: {
    if (kind === "disconnected") return "Disconnected"
    var lines = []
    if (kind === "wifi") lines.push("Network: " + (info.ssid || "Wi-Fi"))
    else if (kind === "ethernet") lines.push("Network: Ethernet")
    if (info.signal !== undefined && info.signal !== "") {
      lines.push("Signal strength: " + info.signal + "%")
    }
    if (info.freq) lines.push("Frequency: " + info.freq + "MHz")
    if (info.iface) lines.push("Interface: " + info.iface)
    if (info.ip) lines.push("IP: " + info.ip + "/" + (info.prefix || ""))
    if (info.gateway) lines.push("Gateway: " + info.gateway)
    if (showBarBandwidth) {
      lines.push("Download: " + barDownloadText)
      lines.push("Upload: " + barUploadText)
    }
    return lines.join("\n")
  }
  ...
  function formatRateCompact(bytesPerSec) {
    return Model.formatRateCompact(bytesPerSec)
  }
  ...
  // Poll network throughput on the bar (Waybar interval: 2s). Panel details
  // polling stays faster while the popup is open.
  Timer {
    id: barPoll
    interval: 2000
    repeat: true
