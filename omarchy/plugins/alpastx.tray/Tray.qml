...
BarWidget {
  ...
  readonly property bool expanded: alwaysExpanded || hoverExpanded
  ...
  readonly property int animationDuration: alwaysExpanded ? 0 : 600
  ...
  function tooltipTargetAt(localX, localY) {
    var host = trayContent.item
    if (!host) return null

    var hostPoint = { x: localX, y: localY }
    try {
      hostPoint = root.mapToItem(host, localX, localY)
    } catch (e) {
      return null
    }

    var items = []
    collectTrayTooltipItems(host, items)
    for (var i = items.length - 1; i >= 0; i--) {
      var item = items[i]
      if (!item || !item.visible || item.width <= 0 || item.height <= 0) continue

      var local = { x: hostPoint.x, y: hostPoint.y }
      try {
        local = host.mapToItem(item, hostPoint.x, hostPoint.y)
      } catch (e) {
        continue
      }
  ...
  function collectTrayTooltipItems(node, out) {
    if (!node) return
    if (node.modelData !== undefined && typeof node.displayMenu === "function") {
      out.push(node)
      return
    }
    ...
    for (var i = 0; i < kids.length; i++)
      collectTrayTooltipItems(kids[i], out)
  }
  ...
  function persistTrayState(pinned, hidden) {
    ...
    var id = root.moduleName || "alpastx.tray"
    ...
    Item {
      ...
      Item {
        ...
        HoverHandler {
          ...
          onHoveredChanged: if (!root.alwaysExpanded) root.hoverExpanded = hovered
        }
        ...
        Item {
          ...
          height: root.barSize
          clip: !root.alwaysExpanded

          Row {
            id: trayIcons
            x: root.alwaysExpanded ? 0 : root.drawerExtent - root.revealExtent
            anchors.verticalCenter: parent.verticalCenter
    ...
    Item {
      ...
      Item {
        ...
        Item {
          ...
          Column {
            id: trayIcons
            y: root.alwaysExpanded ? 0 : root.drawerExtent - root.revealExtent
            anchors.horizontalCenter: parent.horizontalCenter
            spacing: root.trayItemGap
            layer.enabled: true
  ...
  component TrayItem: Item {
    ...
    readonly property string tooltipText: root.trayTooltip(modelData)
  }
}
