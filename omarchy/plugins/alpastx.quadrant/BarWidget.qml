...
BarWidget {
  ...
  // Bar glyphs are icons, not captions: they size with bodySmall so they
  // read at the same weight as neighbouring bar pills.
  readonly property int glyphFontSize: Style.font.bodySmall
  ...
  // Single caption/glyph line. Vertical bars drop the glyph.
  readonly property int lineBoxHeight: {
    if (root.vertical) return pctSizer.implicitHeight
    var g = Math.max(cpuGlyphSizer.implicitHeight, gpuGlyphSizer.implicitHeight, memGlyphSizer.implicitHeight, diskGlyphSizer.implicitHeight)
  ...
  function persistSegments(list) {
    ...
    var quotedId = (typeof Util !== "undefined" && Util.shellQuote)
      ? Util.shellQuote("alpastx.quadrant") : "'alpastx.quadrant'"
  ...
  BarIconButton {
    ...
    fixedWidth: root.vertical
                ...
                : (root.showMonitorFallback
                   ...
                   : segGrid.implicitWidth + root.hPad * 2)
    fixedHeight: root.vertical
                 ? (root.showMonitorFallback
                    ? Style.bar.statusSlot
                    : segGrid.implicitHeight + root.hPad * 2)
                 : -1
  ...
  component MetricCell: Item {
    ...
    readonly property color valueColor: cell.hot ? root.hotTextColor : root.barText
