...
Panel {
  ...
  readonly property var tabs: {
    var all = ["cpu", "gpu", "mem", "disk"]
    if (gpuAvailable) return all
    return all.filter(function (t) { return t !== "gpu" })
  }
  readonly property var tabLabels: ({ "cpu": "CPU", "gpu": "GPU", "mem": "MEMORY", "disk": "DRIVES" })
  ...
  function refreshActiveTab() {
    var map = { "cpu": cpuTab, "mem": memTab, "gpu": gpuTab, "disk": diskTab }
        ...
        Flickable {
          ...
          Column {
            ...
            StackLayout {
              id: stack
              width: parent.width
              // Children are always cpu/mem/gpu/disk in that order. Map by
              // tab id rather than by the filtered `tabs` array — otherwise a
              // no-GPU machine puts Drives at index 2, which is GpuTab.
              currentIndex: {
                var map = { "cpu": 0, "mem": 1, "gpu": 2, "disk": 3 }
