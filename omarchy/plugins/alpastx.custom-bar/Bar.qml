...
Item {
  ...
  function targetInSlot(target, slot) {
    ...
    if (target === slot || target === slot.activeItem) return true
    var node = target
    while (node) {
      if (node === slot.activeItem || node === slot) return true
      node = node.parent
    }
    return false
  }

  function slotHoveringTarget(target) {
    for (var i = 0; i < moduleSlots.length; i++) {
      var slot = moduleSlots[i]
      if (!slot || !slot.hovered || !slot.visible || slot.width <= 0 || slot.height <= 0) continue
      if (targetInSlot(target, slot)) return true
    }
    return false
  }
  ...
  function moduleHasTooltip(target) {
    ...
    if ("tooltipText" in target && String(target.tooltipText || "") !== "") return true
    if ("tooltip" in target && String(target.tooltip || "") !== "") return true
    return false
  }

  function moduleTooltipText(target) {
    if (!target) return ""
    if ("tooltipText" in target && target.tooltipText) return String(target.tooltipText)
    if ("tooltip" in target && target.tooltip) return String(target.tooltip)
    return ""
  }

  function moduleTooltipTargetAt(slot, localX, localY) {
    if (!slot) return null

    var clickTarget = moduleClickTargetAt(slot, localX, localY)
    if (clickTarget && moduleHasTooltip(clickTarget)) return clickTarget

    var item = slot.activeItem
    if (item && typeof item.tooltipTargetAt === "function") {
      var itemPoint = { x: localX, y: localY }
      try {
        itemPoint = slot.mapToItem(item, localX, localY)
      } catch (e) {
      }
      var delegated = item.tooltipTargetAt(itemPoint.x, itemPoint.y)
      if (delegated && moduleHasTooltip(delegated)) return delegated
    }

    if (moduleHasTooltip(item)) return item
    return null
  }
      ...
      Item {
        ...
        Item {
          ...
          readonly property real leftContentWidth: {
            if (leftModules.implicitWidth > 0)
              return leftModules.implicitWidth
            return Math.max(0, leftModules.width)
          }
          implicitWidth: leftContentWidth
          implicitHeight: Math.max(1, root.barSize - root.islandInset * 2)
        ...
        Item {
          ...
          readonly property real rightContentWidth: {
            if (rightModules.implicitWidth > 0)
              return rightModules.implicitWidth
            return Math.max(0, rightModules.width)
          }
          implicitWidth: rightContentWidth
          implicitHeight: Math.max(1, root.barSize - root.islandInset * 2)
      ...
      Item {
        ...
        Item {
          ...
          readonly property real leftContentHeight: {
            if (leftModulesV.implicitHeight > 0)
              return leftModulesV.implicitHeight
            return Math.max(0, leftModulesV.height)
          }
          implicitWidth: Math.max(1, root.barSize - root.islandInset * 2)
          implicitHeight: leftContentHeight
          width: Math.max(1, root.barSize - root.islandInset * 2)
        ...
        Item {
          ...
          readonly property real rightContentHeight: {
            if (rightModulesV.implicitHeight > 0)
              return rightModulesV.implicitHeight
            return Math.max(0, rightModulesV.height)
          }
          implicitWidth: Math.max(1, root.barSize - root.islandInset * 2)
          implicitHeight: rightContentHeight
          width: Math.max(1, root.barSize - root.islandInset * 2)
  ...
  function leftGroupEntries(entries, group) {
    ...
    for (var i = 0; i < entries.length; i++) {
      var id = entryId(entries[i])
      if (allow.indexOf(id) !== -1) result.push(entries[i])
    }
    return result
  }

  function rightGroupEntries(entries, group) {
    var networkIds = ["omarchy.network", "alpastx.network"]
    var ipIds = ["alpastx.ip"]
    var quadrantIds = ["alpastx.quadrant"]
    var powerMenuIds = ["alpastx.power-menu"]
    var mainIds = ["omarchy.agents", "omarchy.bluetooth", "omarchy.audio", "omarchy.monitor", "omarchy.power"]
    var allow = group === "network" ? networkIds
      : group === "ip" ? ipIds
      : group === "quadrant" ? quadrantIds
      : group === "power" ? powerMenuIds
      : mainIds
    var result = []
    if (!Array.isArray(entries)) return result
    for (var i = 0; i < entries.length; i++) {
  ...
  // Left section uses four island capsules: menu, workspaces, status
  // (clock + keyboard layout + system update), then tray at the inner edge.
  // Right uses five: network, IP
  // address, quadrant (cpu/gpu/mem), main modules, and power-menu pill.
  component LeftPillGroup: Item {
    id: leftPillGroupRoot
    ...
    IslandBackdrop {
      anchors.fill: parent
      visible: leftPillGroupRoot.visible && (root.vertical ? leftPillGroupRoot.contentHeight > 0 : leftPillGroupRoot.contentWidth > 0)
    }
  ...
  component LeftModules: Loader {
    id: leftModulesRoot

    property var entries: root.layoutEntries("left")
    property string region: "left"
    readonly property int pillSpacing: Style.space(6)
    readonly property var trayEntries: root.leftGroupEntries(entries, "tray")
    readonly property var menuEntries: root.leftGroupEntries(entries, "menu")
    readonly property var workspacesEntries: root.leftGroupEntries(entries, "workspaces")
    readonly property var statusEntries: root.leftGroupEntries(entries, "status")

    visible: entries.length > 0
    active: visible && entries.length > 0
    sourceComponent: root.vertical ? verticalLeftModules : horizontalLeftModules
    width: item ? item.implicitWidth : 0
    height: item ? item.implicitHeight : 0

    Component {
      id: horizontalLeftModules

      Row {
        spacing: leftModulesRoot.pillSpacing

        LeftPillGroup {
          entries: leftModulesRoot.menuEntries
          region: leftModulesRoot.region
        }

        LeftPillGroup {
          entries: leftModulesRoot.workspacesEntries
          region: leftModulesRoot.region
        }

        LeftPillGroup {
          entries: leftModulesRoot.statusEntries
          region: leftModulesRoot.region
        }

        LeftPillGroup {
          entries: leftModulesRoot.trayEntries
          region: leftModulesRoot.region
        }
      }
    }
  ...
  component RightPillGroup: Item {
    id: rightPillGroupRoot
    ...
    IslandBackdrop {
      anchors.fill: parent
      visible: rightPillGroupRoot.visible && (root.vertical ? rightPillGroupRoot.contentHeight > 0 : rightPillGroupRoot.contentWidth > 0)
    }
  ...
  component RightModules: Loader {
    id: rightModulesRoot

    property var entries: root.layoutEntries("right")
    property string region: "right"
    readonly property int pillSpacing: Style.space(6)
    readonly property var networkEntries: root.rightGroupEntries(entries, "network")
    readonly property var ipEntries: root.rightGroupEntries(entries, "ip")
    readonly property var quadrantEntries: root.rightGroupEntries(entries, "quadrant")
    readonly property var mainEntries: root.rightGroupEntries(entries, "main")
    readonly property var powerEntries: root.rightGroupEntries(entries, "power")

    visible: entries.length > 0
    active: visible && entries.length > 0
    sourceComponent: root.vertical ? verticalRightModules : horizontalRightModules
    width: item ? item.implicitWidth : 0
    height: item ? item.implicitHeight : 0

    Component {
      id: horizontalRightModules

      Row {
        spacing: rightModulesRoot.pillSpacing

        RightPillGroup {
          entries: rightModulesRoot.networkEntries
          region: rightModulesRoot.region
        }

        RightPillGroup {
          entries: rightModulesRoot.ipEntries
          region: rightModulesRoot.region
        }

        RightPillGroup {
          entries: rightModulesRoot.quadrantEntries
          region: rightModulesRoot.region
        }
  ...
  // Each center layout entry gets its own island capsule. When centerAnchor is
  // set, that module stays at the true bar center and before/after entries sit
  // to its left/right (or above/below on vertical bars) as separate pills.
  component CenterPillList: Loader {
    id: centerPillListRoot
    ...
    visible: entries.length > 0
    active: visible && entries.length > 0
    sourceComponent: root.vertical ? verticalCenterPillList : horizontalCenterPillList
    width: item ? item.implicitWidth : 0
    height: item ? item.implicitHeight : 0

    Component {
      id: horizontalCenterPillList

      Row {
        spacing: centerPillListRoot.pillSpacing

        Repeater {
          model: centerPillListRoot.entries

          Item {
            required property var modelData

            implicitWidth: pillContentWidth > 0 ? pillContentWidth + root.islandPad * 2 : 0
            implicitHeight: Math.max(1, root.barSize - root.islandInset * 2)
            width: implicitWidth
            height: implicitHeight
            readonly property real pillContentWidth: {
              if (pillSlot.implicitWidth > 0) return pillSlot.implicitWidth
              return Math.max(0, pillSlot.width)
            }

            IslandBackdrop {
              anchors.fill: parent
              visible: parent.pillContentWidth > 0
            }
            ...
            ModuleSlot {
              ...
              anchors.verticalCenter: parent.verticalCenter
              entry: modelData
              region: centerPillListRoot.region
            }
          }
        }
      }
      ...
      Column {
        ...
        Repeater {
          ...
          Item {
            ...
            implicitWidth: Math.max(1, root.barSize - root.islandInset * 2)
            implicitHeight: pillContentHeight > 0 ? pillContentHeight + root.islandPad * 2 : 0
            width: implicitWidth
            height: implicitHeight
            readonly property real pillContentHeight: {
              if (pillSlot.implicitHeight > 0) return pillSlot.implicitHeight
              return Math.max(0, pillSlot.height)
            }

            IslandBackdrop {
              anchors.fill: parent
              visible: parent.pillContentHeight > 0
            }
  ...
  component CenterAnchorPill: Item {
    id: centerAnchorPillRoot
    ...
    implicitWidth: vertical
      ? Math.max(1, root.barSize - root.islandInset * 2)
      : (pillContentWidth > 0 ? pillContentWidth + root.islandPad * 2 : 0)
    implicitHeight: vertical
      ? (pillContentHeight > 0 ? pillContentHeight + root.islandPad * 2 : 0)
      : Math.max(1, root.barSize - root.islandInset * 2)
    width: implicitWidth
    height: implicitHeight
    readonly property real pillContentWidth: {
    ...
    IslandBackdrop {
      anchors.fill: parent
      visible: centerAnchorPillRoot.vertical ? parent.pillContentHeight > 0 : parent.pillContentWidth > 0
    }
    ...
    Item {
      ...
      ModuleSlot {
        id: pillSlot
        anchors.centerIn: parent
        entry: centerAnchorPillRoot.entry
        region: centerAnchorPillRoot.region
      }
    }
  }
    ...
    Component {
      ...
      Item {
        ...
        CenterPillList {
          ...
          anchors.right: anchorPill.left
          anchors.rightMargin: beforePills.visible && anchorPill.visible ? centerRoot.pillSpacing : 0
          anchors.verticalCenter: anchorPill.verticalCenter
        }
        ...
        CenterPillList {
          ...
          anchors.left: anchorPill.right
          anchors.leftMargin: afterPills.visible && anchorPill.visible ? centerRoot.pillSpacing : 0
          anchors.verticalCenter: anchorPill.verticalCenter
        }
      }
    }
    ...
    Component {
      ...
      Item {
        ...
        CenterPillList {
          ...
          visible: centerRoot.hasAnchor
          entries: root.entriesBefore(centerRoot.entries, root.centerAnchor)
          anchors.bottom: anchorPillV.top
          anchors.bottomMargin: beforePillsV.visible && anchorPillV.visible ? centerRoot.pillSpacing : 0
          anchors.horizontalCenter: anchorPillV.horizontalCenter
        }
        ...
        CenterPillList {
          ...
          visible: centerRoot.hasAnchor
          entries: root.entriesAfter(centerRoot.entries, root.centerAnchor)
          anchors.top: anchorPillV.bottom
          anchors.topMargin: afterPillsV.visible && anchorPillV.visible ? centerRoot.pillSpacing : 0
          anchors.horizontalCenter: anchorPillV.horizontalCenter
        }
      }
    }
  ...
  component ModuleSlot: Item {
    ...
    HoverHandler {
      ...
      onHoveredChanged: slot.syncModuleTooltip()
      onPointChanged: slot.syncModuleTooltip()
    }
    ...
    function syncModuleTooltip() {
      ...
      var point = moduleHover.point.position
      var target = root.moduleTooltipTargetAt(slot, point.x, point.y)
      var text = root.moduleTooltipText(target)
      if (!target || !text) {
        if (slot.moduleTooltipTarget) root.hideTooltip(slot.moduleTooltipTarget)
        slot.moduleTooltipTarget = null
        return
    ...
    property var moduleTooltipTarget: null
