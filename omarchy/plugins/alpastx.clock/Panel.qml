...
Panel {
  id: root
  moduleName: "alpastx.clock"
  ipcTarget: "omarchy.clock"
  manageIpc: false

  property var anchorItem: null

  // Stock omarchy keeps the clock in the center section and centers the
  // calendar on the bar. When the widget lives elsewhere, anchor to the
  // clock button instead.
  readonly property bool centerOnBar: {
    if (!root.bar || !root.bar.layoutConfig) return false
    var center = root.bar.layoutConfig.center
    if (!Array.isArray(center)) return false
    var name = root.moduleName
